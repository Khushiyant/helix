...
from dataset import get_dataloaders, get_speechcommands_dataloaders, get_concat_speechcommands_dataloaders, get_urbansound8k_dataloaders, get_librispeech_dataloaders, get_librispeech_scaling_dataloaders, get_voxpopuli_scaling_dataloaders, get_tedlium_scaling_dataloaders
...
DATASET_CONFIG = {
    ...
    "tedlium": {"num_classes": 2028, "num_folds": 1, "default_root": "data/TEDLIUM"},
}
...
def train_fold(fold, mode, data_root, device, dataset="esc50", epochs=100, batch_size=32, lr=3e-4, n_clips=1, n_pool_tokens=None, use_wandb=False, use_amp=False, grad_accum_steps=1, save_every=0, resume=False, target_seconds=0):
    ...
    if dataset == "tedlium":
        clip_seconds = target_seconds if target_seconds > 0 else 300
        train_loader, test_loader, num_classes = get_tedlium_scaling_dataloaders(
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="esc50",
                        choices=["esc50", "urbansound8k", "speechcommands", "librispeech", "voxpopuli", "tedlium"])
    ...
    if args.dataset not in ("speechcommands", "librispeech", "voxpopuli", "tedlium") and not os.path.exists(args.data_root):
